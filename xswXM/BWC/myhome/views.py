...
def index(request):
	...
	for x in data:
		...
		for v in x.sub:
			...
			onedata.append(v)
	print(onedata)
	context = {'tglist':data,'onedata':onedata}
...
def buy(request):
    ...
    obb = Orders.objects.get(id=orderid)
...
#安全设置
def mysafety(request):

    return render(request,'myhome/word/safety.html')
...
def myaddress(request):
    return render(request,'myhome/word/address.html')
